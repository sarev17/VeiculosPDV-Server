...
<body>
    ...
    <p class="upper">COMPRADOR: {{ $v->cliente }} CPF/CNPJ: {{ $v->cpf }}), localizado na
        {{ $v->endereco }}.</p>
    <p class="indent">Pelo presente instrumento, as partes supra citadas têm entre si justo e contratado a venda
        de um veículo
        <span class="upper"><b>{{ $v->marca }} {{ $v->modelo }}, {{ $v->cor }}, ano
                {{ $v->fabricacao }}, placa {{ $v->placa }}</b></span>, nos seguintes termos:
    </p>
    <p class="indent">1. O veículo é de propriedade do VENDEDOR, que comprova ao COMPRADOR mediante entrega do
        certificado número
        <b>{{ $v->renavam }}</b> expedido pela DETRAN, totalmente livre e desembaraçado de quaisquer ônus.
    </p>
    <p class="indent">2. O preço total do veículo é de R$ {{ number_format($v->total, 2, ',', '.') }} ({{strtoupper(clsTexto::valorPorExtenso($v->total))}})  </p>
    <p class="indent">3. O comprador pagará o veículo nas seguintes condições:</p>
    <p class="indent">3.1 - R$ {{ number_format($v->entrada, 2, ',', '.') }} ({{strtoupper(clsTexto::valorPorExtenso($v->entrada))}}) de entrada mais
        {{ $v->parcelas }} parcelas de
        {{ number_format($v->mensalidade, 2, ',', '.') }} ({{strtoupper(clsTexto::valorPorExtenso($v->mensalidade))}}) , mediante pagamento á vista, representado por (dinheiro ou
        cheque - no
    ...
    <br><br>
    <p class="center">________________________________________________</p>
    <p class="center upper">{{ $v->cliente }} ({{ $v->cpf }})</p>
